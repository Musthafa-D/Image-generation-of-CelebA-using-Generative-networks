...
class CGAN(BaseNetwork):
    def __init__(self, name: str, noise_dim: int, hidden_channels: int, 
                 final_layer: str, debug=False):
...
class Generator(torch.nn.Module):
    def __init__(self, noise_dim: int, hidden_channels: int, num_labels: int):
        super(Generator, self).__init__()
        initial_hidden_channels = hidden_channels*8

        self.gen = torch.nn.Sequential(
            
            torch.nn.Linear(noise_dim + num_labels, initial_hidden_channels*4*4),
            torch.nn.ReLU(),
            ...
            torch.nn.ConvTranspose2d(hidden_channels, hidden_channels//2, 4, 2, 1, bias=False), #64
            torch.nn.BatchNorm2d(hidden_channels//2),
            torch.nn.ReLU(True),
            
            torch.nn.Conv2d(hidden_channels//2, 3, 3, 1, 1, bias=False), #64
            torch.nn.Sigmoid()
        )
...
class Discriminator(torch.nn.Module):
    def __init__(self, hidden_channels: int, num_labels: int, final_layer: str):
        super(Discriminator, self).__init__()
        
        self.dis = torch.nn.Sequential()
        self.dis.append(torch.nn.Conv2d(3 + num_labels, hidden_channels//2, 4, stride=2, padding=1)) #32
        self.dis.append(torch.nn.LeakyReLU(0.2, inplace=True))
        
        self.dis.append(torch.nn.Conv2d(hidden_channels//2, hidden_channels, 4, stride=2, padding=1)) #16
        self.dis.append(torch.nn.BatchNorm2d(hidden_channels))
        self.dis.append(torch.nn.LeakyReLU(0.2, inplace=True))
        ...
        self.dis.append(torch.nn.Conv2d(hidden_channels*4, 12, 4))
...
class Dis(torch.nn.Module):
    def __init__(self, hidden_channels: int, final_layer:str):
        ...
        self.dis = torch.nn.Sequential()
        self.dis.append(torch.nn.Conv2d(3, hidden_channels//2, 4, stride=2, padding=1)) #32
        self.dis.append(torch.nn.LeakyReLU(0.2, inplace=True))
