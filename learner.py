...
class Learner(BaseGANLearning):
    def __init__(self,
        ...
        super().__init__(train_data, test_data, val_data, trial_path, learner_config, data_storage_names=[
    ...
    def _train_epoch(self, train=True):
        ...
        for i, data in enumerate(self.train_data):
            ...
            self.labels = labels.to(self.device).float()
...
class Conditional_Learner(BaseCGANLearning):
    def __init__(self,
        ...
        #for i in range (10):
            #self.plotter.register_default_plot(Label_transition_plot(self, i, label_names[i]))
    ...
    def _test_epoch(self, test=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_data):
                ...
                self.test_real_data = inputs.to(self.device)
                self.test_labels = labels.to(self.device).float()
                real_target = torch.ones(len(inputs), device=self.device)

                # Classify Images
                predictions = self._discriminate(self.test_real_data, self.test_labels)
    ...
    def _hook_every_epoch(self):
        if self.epoch == 0:
            ...
            # Create a noise vector for 30 samples
            self.test_noise = torch.randn(30, self.noise_dim, device=self.device)
    
            # Select 30 labels from the test labels, assuming test_labels is a tensor
            if self.test_labels.size(0) >= 30:
                # Randomly select 30 labels from test_labels
                indices = torch.randperm(self.test_labels.size(0))[:30]
                self.testing_labels = self.test_labels[indices].to(self.device)
            else:
                # If there are fewer than 30 labels, use as many as available
                self.testing_labels = self.test_labels.to(self.device)

        # Saving generated images
        with torch.no_grad():         
            pos_epoch = self.epoch

            generated_images = self._generate(self.test_noise, self.testing_labels)
            predictions_gen = self._discriminate(generated_images.detach(), self.testing_labels)
            
            self.data_storage.dump_store("generated_images", generated_images)
            self.data_storage.dump_store("epochs_gen", pos_epoch)
            self.data_storage.dump_store("predictions_gen", predictions_gen.detach().cpu().numpy())
            self.data_storage.dump_store("test_labels", self.testing_labels)
